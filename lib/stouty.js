...
module.exports.listen = function init (port) {
	...
	server = net.createServer(function(socket){
		...
		socket.write('stouty ready: ' + os.hostname() + (cluster.isMaster ? ' - ' + Object.keys(cluster.workers).length + ' workers online' : '') +  '\r\n');
	});
...
function handleStout(message, level) {
	...
	if(cluster.isWorker) {
		return process.send({ cmd: 'stouty', data: message });
	}
	...
	for (var i = sockets.length - 1; i >= 0; i--) {
		...
		if(sockets[i].level && sockets[i].level != 'all' && sockets[i].level != level) continue;
...
cluster.on('fork', function(worker){
	worker.on('message', function(msg){
		if (msg.cmd !== 'stouty') return;

		handleStout(hostname + ' - ' + msg.data);
	});
});
